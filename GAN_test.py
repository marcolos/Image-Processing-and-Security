...
def division(finalScorePath, opHistoryPath, valore_soglia, valore_optout,returnFlag = 0):
    # finalScore.csv
...
def AddHistory(scoreMatrix,opHistory,lenghtFlag = 0):
    tmpMatrix = []
    tmp = []
    maxlenght = 0   
    for i in range(len(scoreMatrix)):
    ...
    for i in range(len(matrix)):
        for j in range(len(matrix[i][2])):
            for k in range(len(repeatOp)):
                if matrix[i][2][j]['name'] == repeatOp[k][0]:
                    foundOp = True
                    break
            if foundOp == False:
                for t in range(len(matrix)):
                    for s in range(len(matrix[t][2])):
                        if matrix[i][2][j]['name'] == matrix[t][2][s]['name']:
                            countOp = countOp + 1
                            break
            if countOp > 0:
                tmp.append(matrix[i][2][j]['name'])
                tmp.append(countOp)
                repeatOp.append(tmp)
            countOp = 0
            tmp = []
...
def manipulation_analysis(finalScorePath, opHistoryPath,valore_soglia, valore_optout):

    scoreMatrix_opt,MATCHED,NOMATCHED,noJSON = division(finalScorePath, opHistoryPath, valore_soglia, valore_optout,returnFlag = 1)

    with open(opHistoryPath) as f:
        opHistory = json.load(f)
    scoreOperations_one = AddHistory(scoreMatrix_opt,opHistory)
    scoreOperations_no_one_matched = AddHistory(MATCHED,opHistory)
    scoreOperations_no_one_no_matched = AddHistory(NOMATCHED,opHistory)
...
def scoreFilter1(finalScorePath, opHistoryPath, score, opFilter, operator='null', score2 = None):
    ...
    if duplicateOP == False:
        tmpMatrix,maxlenght = AddHistory(scoreMatrix,opHistory,lenghtFlag = 1)
        result = []
        tmpMatrix2 = []
        occurenceMatrix = []
...
def scoreFilter2(finalScorePath, opHistoryPath, score, opFilter, operator='null', score2=None):
    ...
    if duplicateOP == False:
        tmpMatrix = AddHistory(scoreMatrix,opHistory)
        countOperations = 0
        previousOperations = []
        finalResult = []
        result = []
...
def main():
    ...
    if args.subcommand == 'create-json-probe-history':
        ...
        manipulation_analysis(finalScorePath=args.scorepath, opHistoryPath=args.probehistory, valore_soglia=args.valoresoglia, valore_optout=args.valoreoptout)
...
def mainInterattivo():
    ...
    if (mainScelta < 1) or (mainScelta > 6):
        ...
        valore_soglia = float(input())
        print('insert valore di optout: ')
        valore_optout = float(input())

        manipulation_analysis(finalScorePath=finalScorePath, opHistoryPath=opHistoryPath, valore_soglia=valore_soglia, valore_optout=valore_optout)
